    ...
    def test_sep(self):
        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(data_loader):
                ...
                n = 0
                # c_trg_list = self.create_labels(c_org[idx], self.c_dim, self.dataset, self.selected_attrs)
                for c_trg in c_trg_list:
                    # x_fake_list.append(self.G(x_real, c_trg))
                    x_fake_list2.append(self.G(x_real, c_trg))
                    # x_fake_list2 = self.G(x_real, c_trg)
                    
                    xcat = torch.cat(x_fake_list2, dim=3)
                    path = self.result_dir+"/"+cat_dict.get(n)+"/"
                    os.makedirs(path, exist_ok = True)
                    # catpath = os.path.join(path, f'{self.file_name}.jpg')
                    catpath = os.path.join(path, data_loader.dataset.imgs[i][0].split("/")[-1])
                    save_image(self.denorm(xcat.data.cpu()), catpath, nrow=1, padding=0)
                    # print('Saved {} images in {}'.format(data_loader.dataset.imgs[i][0].split("/")[-1], catpath))
                    n+=1
                    x_fake_list2 = []
                    
                print('Images {} is complete.'.format(data_loader.dataset.imgs[i][0].split("/")[-1]))
                
                
                # for j in range(len(x_real)):
                #     # Prepare input images and target domain labels.
                #     x_real = x_real[[j]].to(self.device)
                #     c_trg_list = self.create_labels(c_org[[j]], self.c_dim, self.dataset, self.selected_attrs)
                #     # Translate images.
                #     # x_fake_list = [x_real]
                #     x_fake_list2 = []
